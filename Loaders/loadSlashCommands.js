...
module.exports = async bot => {
  ...
  await rest.put(Routes.applicationCommands(bot.user.id), {body: commands})
    setInterval(async () => {
        const Statut_Random_AutoChange = [`${bot.users.cache.size} membres`,`${bot.guilds.cache.size} serveurs`,`la version 0.4`][Math.floor(Math.random() * [`${bot.guilds.cache.size} serveurs`,`${bot.users.cache.size} membres`,`la version 0.4`].length)];
